...
//  WZMarqueeLabel
//
//  Created by LiuSky on 04/16/2020.
...
open class MarqueeLabel: UILabel, CAAnimationDelegate {
    ...
    /// 默认为（0）也就是无限滚动
    public var maxNumOfScrolls: Int = 0
    
    /// 当前滚动次数
    public var currentNumOfScrolls = 0
    
    public typealias AnimationCompletionBlock = (_ finished: Bool) -> Void
    /// 一次动画完成之后的回调
    public var animationCompletionBlock: AnimationCompletionBlock?
    ...
    */
    ...
    open func labelReturnedToHome(_ finished: Bool) {
        // 如果没有设置最大滚动，那么我们不需要停止滚动
        guard self.maxNumOfScrolls > 0 else { return }
        
        // 增加滚动数
        self.currentNumOfScrolls += 1
        
        // 如果当前滚动的数目大于或等于最大停止滚动
        if self.currentNumOfScrolls >= self.maxNumOfScrolls {
            self.holdScrolling = true
            self.currentNumOfScrolls = 0
